...
class Cernlib(Package):
    ...
    for v in versions:
        ...
        resource(
            ...
            url="http://www-zeuthen.desy.de/linear_collider/cernlib/new/cernlib.2005.corr.{}.tgz".format(
                v
            ),
            sha256=versions[v][0],
        ...
        resource(
            ...
            url="http://www-zeuthen.desy.de/linear_collider/cernlib/new/cernlib.2005.install.{}.tgz".format(
                v
            ),
            sha256=versions[v][1],
            destination="resources",
    ...
    def unpack(self, spec, prefix):
        ...
        install_tar = tarfile.open("resources/install/cernlib.2005.install.2020.12.11.tgz")
        install_tar.extractall()

        # Update corr tar file
        copy("resources/corr/cernlib.2005.corr.2020.12.11.tgz", "cernlib.2005.corr.tgz")
        ...
        def patch(target_file, patch_file):
            which("patch")("-N", "-l", "-p1", target_file, "-i", join_path(patches, patch_file))
    ...
    def install(self, spec, prefix):
        ...
        for dir in ["bin", "lib", "include"]:
            install_tree(join_path(level, dir), join_path(prefix, dir))
